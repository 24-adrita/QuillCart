...
public class MainWindow extends JFrame {
    ...
    // Panel to display aggregated customer information from past purchases
    private final CustomerRecordsPanel customerRecordsPanel = new CustomerRecordsPanel(this);
    ...
    // ===== Pending customer info (collected at checkout) =====
    private String pendingCustomerName;
    private String pendingCustomerPhone;
    private String pendingCustomerAddress;
    ...
    public MainWindow() {
        ...
        // Register customer records view
        mainPanel.add(customerRecordsPanel, "CustomerRecords");
    ...
    public Purchase addPurchase(Map<Book, Integer> cart, double totalTaka, boolean promoApplied, String promo) {
        return addPurchase(cart, totalTaka, promoApplied, promo, "", "", "");
    }

    /**
     * Overloaded convenience allowing customer details to be stored along
     * with the purchase. When supplied, the purchaser's name, phone and
     * address will be persisted so receipts and customer records can
     * accurately reflect who placed the order.
     */
    public Purchase addPurchase(Map<Book, Integer> cart, double totalTaka, boolean promoApplied,
                                String promo, String customerName, String customerPhone, String customerAddress) {
    ...
    /**
     * Opens the customer records view, refreshing its contents before
     * displaying. This method is typically invoked from the owner
     * console when the shopkeeper wishes to see all customers who have
     * made purchases.
     */
    public void openCustomerRecords() {
        customerRecordsPanel.refresh();
        switchPanel("CustomerRecords");
    }

    /**
     * Exposes the customer records panel for internal use. External
     * classes should prefer {@link #openCustomerRecords()} which will
     * automatically refresh the content.
     */
    CustomerRecordsPanel getCustomerRecordsPanel() {
    ...
    /**
     * Stores the customer details associated with the next pending order.
     * These values are reset after the order is finalised. Passing
     * {@code null} values is safe and will be converted to empty strings
     * when creating the {@link Purchase}.
     *
     * @param name  purchaser name
     * @param phone purchaser phone number
     * @param address purchaser shipping address
     */
    public void setPendingCustomerInfo(String name, String phone, String address) {
        this.pendingCustomerName = name;
        this.pendingCustomerPhone = phone;
        this.pendingCustomerAddress = address;
    }
    ...
    public void finalizePendingOrderAndPrint() {
        ...
        // Save purchase with captured customer details
        String custName = (pendingCustomerName == null ? "" : pendingCustomerName);
        String custPhone = (pendingCustomerPhone == null ? "" : pendingCustomerPhone);
        String custAddr = (pendingCustomerAddress == null ? "" : pendingCustomerAddress);
        Purchase purchase = addPurchase(new LinkedHashMap<>(pendingCart), total, pendingPromoApplied,
                pendingPromoCode, custName, custPhone, custAddr);
   ...
   public CustomerPanel getCustomerPanel() {
    ...
    private String buildReceiptText(Purchase p) {
        // Delegate to PurchaseHistoryPanel's formatter to ensure consistent style
        return PurchaseHistoryPanel.buildReceiptText(p);
    }

    private void showReceiptDialog(String receiptText) {
        // Use the custom ReceiptDialog class for a polished presentation
        ReceiptDialog dialog = new ReceiptDialog(this, receiptText);
